...
(function ($) {
    ...
    //滚动时TAB栏切换,置位
    $.toggleTabTop = function() {
        if($.custom.toggleTabTop) {
            $.custom.pageObj.find('.content').scrollTop($.custom.toggleTabTop);
        }
    }
})(Zepto);
...
(function ($) {
    ...
    if (android) {
        ...
        if (ua.toLowerCase().indexOf('chrome') >= 0) {
            ...
            var chrome = ua.match(/(Chrome);?[\s\/]+([\d.]+)?/);
            device.osVersion2 = chrome[2];

            if (parseFloat(device.osVersion2) >= 26.0) {
                $.custom.supportVWVH = true;
            }

        } else {
            device.androidChrome = false;

            if (parseFloat(device.osVersion) >= 4.4) {
                $.custom.supportVWVH = true;
            }
        }
    }
    ...
    if (device.ios && parseFloat(device.osVersion) >= 6.0) {
        $.custom.supportVWVH = true;
    }
...
+function ($) {
    ...
    $.openSuperModal = function(className) {
        if(hasOpenSuperModal()) $.closeSuperModal();

        var modal = $('.super-modal-overlay');
        if(className) {
            modal.attr('data-item',className);
        }
    ...
    $.closeSuperModal = function() {
     ...
     $.alert = function (content,options) {
        options = options || {};
        var super_modal_overlay = $('.super-modal-overlay');
        var alert = super_modal_overlay.find('.modal-alert');
        alert.find('.alert-content').html(content || '');

         var button = options.button || {};
         var alert_button = alert.find('.alert-button');
         alert_button.html(button.text || '确定');

         alert_button.off('click').on('click', function () {
             $.closeSuperModal();
             if(button && typeof button['callback'] === 'function') button['callback']();
         });

         var alert_title = alert.find('.alert-title');
        if(options.title) {
            alert_title.html(options.title || '提示').show();
        } else {
            alert_title.hide();
        }

        $.openSuperModal('modal-alert');
    };
    $.confirm = function (content, options) {
        options = options || {};

        var confirm = $('.super-modal-overlay').find('.modal-confirm');
        confirm.find('.confirm-content').html(content || '');

        var buttonOk = options.buttonOk || {};
        var buttonCancel = options.buttonCancel || {};

        confirm.find('.confirm-btn-ok').html(buttonOk['text'] || '确定').off('click').on('click', function () {
            $.closeSuperModal();
            if(typeof buttonOk['callback'] === 'function') buttonOk['callback']();
        });

        confirm.find('.confirm-btn-cancel').html(buttonCancel['text'] || '取消').off('click').on('click', function () {
            $.closeSuperModal();
            if(typeof buttonCancel['callback'] === 'function') buttonCancel['callback']();
        });

        var confirm_title = confirm.find('.confirm-title');
        if(options.title) {
            confirm_title.html(options.title || '提示').show();
        } else {
            confirm_title.hide();
        }

        $.openSuperModal('modal-confirm');
    };
    ...
    $.openModal = function (modal, cb) {
        ...
        var isSuperModal = modal.hasClass('super-modal-overlay');
    ...
    $.closeModal = function (modal) {
        ...
        var isModal = modal.hasClass('modal'),
            isSuperModal = modal.hasClass('super-modal-overlay'),
...
+function ($) {
    ...
    var Calendar = function (params) {
        ...
        p.initCalendarEvents = function () {
            ...
            p.container.find('.picker-calendar-prev-month').off('click').on('click', p.prevMonth);
            p.container.find('.picker-calendar-next-month').off('click').on('click', p.nextMonth);
            p.container.find('.picker-calendar-prev-year').off('click').on('click', p.prevYear);
            p.container.find('.picker-calendar-next-year').off('click').on('click', p.nextYear);
            ...
            // 选择年份
            p.container.find('.current-year-value').off('click').on('click', handleYearSelector);

            // 选择月份
            p.container.find('.current-month-value').off('click').on('click', handleMonthSelector);
            /**
             * End - edit by JSoon
             */

            p.wrapper.off('click').on('click', handleDayClick);
            if (p.params.touchMove) {
                 ...
                 * Start - edit by JSoon
                 */
                p.yearsPickerWrapper.off($.touchEvents.start).on($.touchEvents.start, handleYearTouchStart);
                p.yearsPickerWrapper.off($.touchEvents.move).on($.touchEvents.move, handleYearTouchMove);
                p.yearsPickerWrapper.off($.touchEvents.end).on($.touchEvents.end, handleYearTouchEnd);
                /**
                 * Start - edit by JSoon
                 */

                p.wrapper.off($.touchEvents.start).on($.touchEvents.start, handleTouchStart);
                p.wrapper.off($.touchEvents.move).on($.touchEvents.move, handleTouchMove);
                p.wrapper.off($.touchEvents.end).on($.touchEvents.end, handleTouchEnd);
            }
        ...
        function openOnInput(e) {
            ...
            if ($.custom.calendar_opened_flag) return;
            p.open();
        ...
        if (p.params.input) {
            ...
            if (p.input.length > 0) {
                ...
                if (!p.inline) {
                    p.input.off('click').on('click', openOnInput);
                }
                /**
        ...
        if (!p.inline) $('html').off('click').on('click', closeOnHTMLClick);
        ...
        p.open = function () {
            ...
            // Set flag
            $.custom.calendar_opened_flag = true;
            p.initialized = true;
            if (p.params.onMonthAdd) {
...
+function (window) {
    ...
    IScroll.prototype = {
        ...
        resetPosition: function (time) {
            ...
            if (this.options.ptr && this.y > 44 && this.startY * -1 < getScreenHeight() && !this.ptrLock) {// jshint ignore:line
        ...
        _initIndicators: function () {
            ...
            if (this.options.scrollbars) {
                ...
                if (this.options.scrollX) {
                    ...
                    $(this.wrapper).find('.iScrollLoneScrollbar').remove();
                    this.wrapper.appendChild(indicator.el);
                    indicators.push(indicator);
                }
            }
...
+function ($) {
    ...
    var Scroller = function (pageContent, _options) {
        ...
        //var useJSScroller = (type === 'js') || (type === 'auto' && ($.device.android && $.compareVersion('4.4.0', $.device.osVersion) > -1) || (type === 'auto' && $.device.ios));
        var useJSScroller = $.useJSScroller($pageContent);
        if (useJSScroller) {
            ...
            if ($pageContent.hasClass('pull-to-refresh-content')) {
                ...
                var minHeight = getScreenHeight() + ($pageContent.prev().hasClass(".bar") ? 1 : 61);
                $pageContent.find('.content-inner').css('min-height', minHeight + 'px');
            }

            var ptr = $(pageContent).hasClass('pull-to-refresh-content');
            //js滚动模式，用transform移动内容区位置，会导致fixed失效，表现类似absolute。因此禁用transform模式（不适用，所以直接改为TRUE）
            //var useTransform = $pageContent.find('.fixed-tab').length === 0;
            var useTransform = true;
            var options = {
                // 1 : 滚动不繁忙的时候触发,2 : 滚动时每隔一定时间触发,3 : 每滚动一像素触发一次
                probeType: this.options.probeType?this.options.probeType:3,
                mouseWheel: true,
            ...
            $pageContent.addClass('native-scroll');
            $.initPullToRefresh = $.initPullToRefreshNative;
            this._bindEventToDomWhenJs();
        }
    };
    Scroller.prototype = {
            ...
            if (this.scroller) {
                ...
                this.scroller.on('scrollEnd', function () {
                    ...
                    try {
                        ...
                        var func = eval('pse_'+page.attr('id'));
    ...
    $.toggleCurInnerScroll = function (page) {
        if(!page) page = $.custom.pageObj;
        page.find('.cur-inner-scroll').removeClass('cur-inner-scroll');
        page.find('.swiper-container .swiper-slide-active').find('.inner-scroll').addClass('cur-inner-scroll');
    }
    //刷新内联滚动
    $.refreshInnerScroller = function(content) {
        if(!content) return;
        if(content.length == 0) return;
        var page = $.custom.pageObj;
        $.toggleCurInnerScroll(page);
        $.toggleFixedTop();
    ...
    $.refreshScroller = function (content) {
        ...
        var check_inner = content.attr('data-check-inner');
        if(!check_inner) check_inner = 'true';
        var innerScroll = content.find('.cur-inner-scroll');
        if(check_inner == 'true' && innerScroll.length > 0) {
            $.refreshInnerScroller($(innerScroll[0]));
        } else {
            var scrollHeight = content[0].scrollHeight;
            var offsetHeight = content.offset().height;
            $.toggleFixedTop();
    ...
    //图片懒加载功能，带有 .lazy-load 的img 进行加载
    $.lazyLoad = function () {
        var offsetTop = getScreenHeight();
        ...
        $('img.lazy-load').each(function () {
            ...
            var top = offset.top;
            var bottom = top + offset.height;
            if (bottom >= 0 && top <= offsetTop) {
                obj.attr("src", obj.attr('data-src'))
                    .removeClass('lazy-load')
                    .addClass('lazy-load-end')
                    .error(function (e) {
                        obj.addClass('lazy-load-error').attr('src', MY_SERVER['PUBLIC'] + '/images/public/blank.png');
                    });
            }
        });
        ...
        $('.bg-lazy-load').each(function () {
            ...
            if (bottom >= 0 && top <= offsetTop) {
                obj.css('background-image',"url("+obj.attr('data-src')+")")
                    .removeClass('bg-lazy-load')
                    .addClass('bg-lazy-load-end');
            }
        });
    ...
    //重置错误的懒加载图片
    $.resetErrorLazyLoad = function() {
        $('img.lazy-load-error').each(function () {
            $(this).removeClass('lazy-load-error')
                .removeClass('lazy-load-end')
                .addClass('lazy-load');
        });

        $('img.swiper-lazy-error').each(function () {
            $(this).removeClass('swiper-lazy-error')
                .removeClass('swiper-lazy-loaded');
        });
    };
    //auto的type,系统版本的小于4.4.0的安卓设备和系统版本小于6.0.0的ios设备，启用js版的iscoll
    $.useJSScroller = function(pageContent) {
        var type = pageContent.attr('data-type');
        return (type === 'js') || (type === 'auto' && ($.device.android && $.compareVersion('4.4.0', $.device.osVersion) > -1) || (type === 'auto' && ($.device.ios && $.compareVersion('6.0.0', $.device.osVersion) > -1)));
    };
    $.topBarShadow = function() {
        var page = $.custom.pageObj;
        var page_content = page.find('.content');
        var shadowHeight = parseInt(page.attr('data-top-shadow-height'));
        if(!shadowHeight) shadowHeight=200;
        var content_inner = page_content.find('.content-inner');
        if(content_inner.length <= 0) {
            return;
        }

        var useJSScroller = $.useJSScroller(page_content);

        var outer_bar = page.find('.outer-bar');
        if(outer_bar.length <= 0) {
            return;
        }

        var inner_bar = page.find('.inner-bar');
        var status_bar = page.find('.i-status-bar');

        if(null === outer_bar.attr('data-opacity')) {
            outer_bar.attr('data-opacity',0);
        }

        if(null === inner_bar.attr('data-opacity')) {
            inner_bar.attr('data-opacity',1);
        }

        var contentTop = -content_inner.offset().top;
        //var outer_bar_opacity = contentTop/shadowHeight;
        //if(outer_bar_opacity < 0) outer_bar_opacity = 0;
        //if(outer_bar_opacity > 1) outer_bar_opacity = 1;
        //var inner_bar_opacity = 1-(contentTop/shadowHeight);
        //if(inner_bar_opacity < 0) inner_bar_opacity = 0;
        //if(inner_bar_opacity > 1) inner_bar_opacity = 1;
        //outer_bar.attr('data-opacity',outer_bar_opacity).css({opacity:outer_bar_opacity});
        //inner_bar.attr('data-opacity',inner_bar_opacity).css({opacity:inner_bar_opacity});
        if(useJSScroller) {
            //标题栏渐变功能效果
            //var contentTop = -page.find('.content-inner').offset().top;
            var outer_bar_opacity = contentTop/shadowHeight;
            if(outer_bar_opacity < 0) outer_bar_opacity = 0;
            if(outer_bar_opacity > 1) outer_bar_opacity = 1;
            var inner_bar_opacity = 1-(contentTop/shadowHeight);
            ...
            if(inner_bar_opacity > 1) inner_bar_opacity = 1;
            outer_bar.attr('data-opacity',outer_bar_opacity).css({opacity:outer_bar_opacity});
            status_bar.css({opacity:outer_bar_opacity});
            inner_bar.attr('data-opacity',inner_bar_opacity).css({opacity:inner_bar_opacity});
        } else {
            //var contentTop = -page.find('.content-inner').offset().top;
            if(contentTop > shadowHeight) {
                if(0 == outer_bar.attr('data-opacity')) {
                    //outer_bar.attr('data-opacity',1).velocity(
                    //    {opacity: 1},
                    //    {
                    //        'easing': 'ease-out',
                    //        'duration': 200
                    //    });
                    outer_bar.attr('data-opacity',1).css({opacity:1});
                    status_bar.css({opacity:1});
                }

                if(1 == inner_bar.attr('data-opacity')) {
                    //inner_bar.attr('data-opacity',0).velocity(
                    //    {opacity: 0},
                    //    {
                    //        'easing': 'ease-out',
                    //        'duration': 200
                    //    });
                    inner_bar.attr('data-opacity',0).css({opacity:0});
                }
            } else {
                if(1 == outer_bar.attr('data-opacity')) {
                    //outer_bar.attr('data-opacity',0).velocity(
                    //    {opacity: 0},
                    //    {
                    //        'easing': 'ease-in',
                    //        'duration': 200
                    //    });
                    outer_bar.attr('data-opacity',0).css({opacity:0});
                    status_bar.css({opacity:0});
                }

                if(0 == inner_bar.attr('data-opacity')) {
                    //inner_bar.attr('data-opacity',1).velocity(
                    //    {opacity: 1},
                    //    {
                    //        'easing': 'ease-in',
                    //        'duration': 200
                    //    });
                    inner_bar.attr('data-opacity',1).css({opacity:1});
                }
            }
        }
    };
    $.initFixedTabTop = function (page) {
        if(!page) page = $.custom.pageObj;
        //页面初始TAB栏top
        var page_data_id = page.attr('data-id');
        if(!$.custom.fixedTabTop[page_data_id]) {
            var buttons_tab = page.find('.content .buttons-tab');
            if (buttons_tab.length > 0) {
                var tabOffset = buttons_tab.offset();
                $.custom.fixedTabTop[page_data_id] = tabOffset.top - tabOffset.height - parseInt(MY_VAR['CLIENT_BAR_HEIGHT']) + 2;    //加2px误差值
            }
        }
    };
    $.getFixedTopContent = function(page) {
        if(!page) page = $.custom.pageObj;
        var cur_inner = page.find('.cur-inner-scroll');
        if(cur_inner.length > 0) {
            return cur_inner;
        }

        return page.find('.content');
    };
    $.toggleFixedTop = function () {
        var page = $.custom.pageObj;
        var fixed_top = page.find('.fixed-top');
        if(fixed_top.length > 0) {
            var beyond = parseInt(fixed_top.attr('data-beyond'));
            if (!beyond) beyond = 2000;

            var bottom = parseInt(fixed_top.attr('data-bottom'));
            if (!bottom) bottom = 4;

            var content = page.find('.cur-inner-scroll');
            if(content.length == 0) {
                content = page.find('.content');
            }

            var scroll_height = content[0].scrollHeight;
            var scroll_bottom = content.scrollTop()+content.offset().height;
            if(scroll_height > beyond && scroll_bottom > beyond) {
                if (fixed_top.css("display") == "none") {
                    fixed_top.css('bottom',bottom+'rem').show().velocity(
                        {opacity: 0.8},
                        {
                            'easing': 'ease-out',
                            'duration': 300
    ...
    //初始化swiper滚动栏宽度
    $.initSwiperBar = function(page) {
        if(!page) page = $.custom.pageObj;
        var swiper_bar = page.find('.swiper-bar');
        var under = parseInt(swiper_bar.attr('data-under-bar'));
        if(!under) under = 0;
        swiper_bar.attr('data-slides-per-view','auto');
        var slide_list = swiper_bar.find('.swiper-slide');
        if(slide_list.length <= under) {
            slide_list.width(getScreenWidth()/slide_list.length);
        } else {
            slide_list.each(function(index,item){
                item = $(item);
                item.width(item.find('span').offset().width+$.custom.remRadix/1.2);
            });
        }
    };
    $.initSwiperHeight = function(page) {
        if(!page) {
            page = $.custom.pageObj;
        }
        page.find('.swiper-slide').each(function(index,item){
            var obj = $(item);
            var height = obj.offset().height;
            var minHeight = getScreenHeight()-obj.parents('.swiper-container').offset().top;
            if(height < minHeight) {
                height = minHeight;
            }
            obj.attr('data-height',height).css('height',height);
        });
    };
    //设置空内容高度
    $.initEmptyContent = function(pageContent) {
        if(!pageContent) {
            pageContent = $.custom.pageObj.find('.content');
        }
        var tab_min_height = getScreenHeight()
            - pageContent.find('.bar.bar-nav').height()
            - pageContent.find('.buttons-tab.fixed-tab').height()
            - pageContent.find('.bar.bar-tab').height();
        pageContent.find('.tab').css('min-height',tab_min_height);
        pageContent.find('.tab .content-empty').css('height',tab_min_height);
        //$.refreshScroller();
    }
...
+function ($) {
    ...
    var showTab = function (tab, tabLink, force) {
        ...
        var currentPage = $.custom.pageObj;
        var currentContent = currentPage.find('.content');

        //记录和跳转到tab的历史位置
        var scrollTop = currentContent.scrollTop();
        oldTab.attr('data-scroll', scrollTop);
        ...
        var newScroll = parseInt(newTab.attr('data-scroll'));
        if (!newScroll) {
            newScroll = 0;
        }

        var tabOffset = currentContent.find('.buttons-tab').offset();
        if (tabOffset.top <= tabOffset.height + parseInt(MY_VAR['CLIENT_BAR_HEIGHT'])) {
            var page_data_id = currentPage.attr('data-id');
            if (newScroll < $.custom.fixedTabTop[page_data_id]) {
                $.custom.toggleTabTop = $.custom.fixedTabTop[page_data_id];
            } else {
                $.custom.toggleTabTop = newScroll;
            }
            $.toggleTabTop();
        }
    ...
    $(document).on("click", ".tab-link", function (e) {
        e.preventDefault();
        var clicked = $(this);
        $('[data-tab-id=' + clicked.attr('data-tab-id') + ']').each(function () {
...
+function ($) {
    ...
    var FixedTab = function (pageContent, _options) {
        ...
        var $pageContent = this.$pageContent = $(pageContent);
        var fixedTop = $pageContent.attr('data-fixed-top');
        if (!fixedTop) {
            fixedTop = $pageContent[0].getBoundingClientRect().top;
            $pageContent.attr('data-fixed-top', fixedTop);
        }

        //var shadow = $pageContent.clone();
        //shadow.css('visibility', 'hidden');

        var replace = $('[data-alternate=' + _options.replacepart + ']');
        if(!replace.html()) {
            replace.html($pageContent.html());
            if($('.i-status-bar').length > 0) {
                replace.css('top',MY_VAR['CLIENT_BAR_HEIGHT']+'px');
            }

            replace.css('margin-top', _options.offset+'rem').css('z-index', 9).hide();
        }
    ...
    FixedTab.prototype = {
        ...
        _scrollHandler: function (ev) {
            ...
            var offset = this.options.offset;
            //var fixedTop = this.options.fixedTop;
            //var fixedScrollTop = fixedTop - offset;
            //var scrollBottom = scrollTop + window.screen.height;
            //var scrollHeight = $scroller[0].scrollHeight;

            //if(scrollTop < 0 || scrollBottom >= scrollHeight) {
            //    return;
            //}

            if (replace) {
                var contentOffset = this.$pageContent.offset();
                var replaceOffset = offset + contentOffset.height;
                if($('.i-status-bar').length > 0) {
                    replaceOffset += parseInt(MY_VAR['CLIENT_BAR_HEIGHT']);
                }

                if (contentOffset.top + 2 < replaceOffset) {    //+2是为了补齐小数点的偏差
                    replace.css('visibility', 'visible').css('display', '');
            ...
            //
            //var isFixed = scrollTop >= fixedScrollTop;
            //if (replace) {
            //    if (isFixed) {
            //        replace.css('visibility', 'visible').css('display', '');
            //        //$pageContent.css('visibility', 'hidden');
...
+function ($) {
    ...
    var initPullToRefreshJS = function (pageContainer) {
        ...
        var page = eventsTarget.hasClass('content') ? eventsTarget : eventsTarget.parents('.content')[0];
        var scroller = $.getScroller(page);
        if (!scroller) return;
...
+function ($) {
    'use strict';
    $.initPullToRefreshNative = function (pageContainer) {
        var eventsTarget = $(pageContainer);
        if (!eventsTarget.hasClass('pull-to-refresh-content')) {
            eventsTarget = eventsTarget.find('.pull-to-refresh-content');
        }
...
+function ($) {
    ...
    $.initSwipePanels = function () {
        ...
        function handleTouchStart(e) {
            $.custom.toggleTabTop = 0;
            startTimer('nativeScrollEnd', function(){$.lazyLoad();}, 500, 10);
        ...
        function handleTouchEnd(e) {
            //$.toggleTabTop();
            $.toggleFixedTop();
            //native使用定时器实现，不是很推荐，（暂定，后期有更好的方案在修改）
            startTimer('nativeScrollEnd', function(){nativeScrollEnd();}, 1000, 5); //单次滑动最多不会超过5秒，所以5秒后停止滚动定时器

            function nativeScrollEnd() {
                var page = $.custom.pageObj;
                var cur_scroll_top = page.find('.content').scrollTop();
                if ($.custom.scrollTop != cur_scroll_top) {
                    $.custom.scrollTop = cur_scroll_top;
                } else {
                    $.custom.scrollTop = -1;
                    $.topBarShadow();
                }
...
+function ($) {
    ...
    var Router = function () {
        ...
        this._init();
        this.xhr = null;
        if(!$('body').hasClass('disable-router')) {
     ...
     * 调用 history.back()
     */
    Router.prototype.back = function (i) {
        $.custom.backFlag = true;
        //theHistory.back(i||-1);
        theHistory.go(i||-1);
    };
     ...
     * @deprecated
     */
    Router.prototype.loadPage = function(url,direction) {
        this.load(url,undefined,undefined,direction);
    };
    ...
    Router.prototype._switchToDocument = function (url, ignoreCache, isPushState, direction) {
        ...
        if(cacheDocument && $.inArray(cacheDocument.pageId, $.custom.correctPages) > -1) {
            $.custom.correctPages.remove(cacheDocument.pageId);
            delete this.cache[baseUrl];
        ...
        if (cacheDocument) {
            this._doSwitchDocument(url,cacheDocument.$content, isPushState, direction);
        } else {
            this._loadDocument(url, {
                success: function ($doc) {
                    try {
                        //context._parseDocument(url, $doc);
                        context._doSwitchDocument(url,$doc.find('.' + routerConfig.sectionGroupClass), isPushState, direction);
                    } catch (e) {
    ...
    Router.prototype._doSwitchDocument = function (url, content, isPushState, direction) {
        ...
        if($.custom.backFlag) {
            if($.custom.curDirection == DIRECTION.direct) {
                direction = DIRECTION.direct;
            } else if($.custom.curDirection == DIRECTION.rightToLeft) {
    ...
    Router.prototype._saveDocumentIntoCache = function (doc, url) {
        this.cache[Util.toUrlObject(url).base] = {
            //$doc: $doc,
    ...
    Router.prototype._animateDocument = function ($from, $to, $visibleSection, direction) {
        ...
        if (direction == DIRECTION.direct) {
            from_end();
            setTimeout(to_end,100);
            //to_end();
        } else {
            this._animateElement($from, $to, direction);
            $from.animationEnd(from_end);
            $to.animationEnd(to_end);
        }
    };
     ...
     * @private
     */
    Router.prototype._deleteCache = function (url) {
        if('-' == url) {
            var obj = this;
            $.each(obj.cache,function(index,item) {
                if(index != getMqstCookie('sq_index_url')) {
                    delete obj.cache[index];
                }
            });
        } else {
    ...
    Router.prototype._back = function (state, fromState) {
        var _this = this;
        if (_this._isTheSameDocument(state.url.full, fromState.url.full)) {
            var $newPage = $('#' + state.pageId);
            if ($newPage.length) {
                var $currentPage = _this._getCurrentSection();
                _this._animateSection($currentPage, $newPage, DIRECTION.leftToRight);
                _this._saveAsCurrentState(state);
            } else {
                location.href = state.url.full;
            }
        } else {
            //this._saveDocumentIntoCache($(document), fromState.url.full);
            _this._switchToDocument(state.url.full, false, false, DIRECTION.leftToRight);
            setTimeout(function(){
                _this._saveAsCurrentState(state);
                _this._deleteCache(Util.toUrlObject(fromState.url.full).base);
            },100);

            //_this._saveAsCurrentState(state);
            //_this._deleteCache(Util.toUrlObject(fromState.url.full).base);
            //sessionStorage.removeItem(_this.sessionNames.currentState);
        }
    };
    ...
    $(function () {
        ...
        $(document).on('click', 'a', function (e) {
            ...
            //需要检测登录
            if($target.attr('data-check-login') == 1) {
            ...
            if ($target.hasClass('back')) {
                ...
                if ($target.attr('data-effect') === 'right') {
                    router.load(url, ignoreCache, undefined, DIRECTION.leftToRight);
                } else if ($target.attr('data-effect') === 'direct') {
                    router.load(url, ignoreCache, undefined, DIRECTION.direct);
                } else {
                    router.load(url, ignoreCache, undefined, DIRECTION.rightToLeft);
                }
            }
        });
    });
...
+function ($) {
    ...
    $.lastPosition = function (options, id, arg) {
        ...
        $(window).off('beforePageSwitch').on('beforePageSwitch', function (event, id, arg) {
            //updateMemory(id, arg);
            updateMemory(arg.attr('data-id'), arg);
        });
        $(window).off('pageAnimationStart').on('pageAnimationStart', function (event, id, arg) {
            //getMemory(id, arg);
            getMemory(arg.attr('data-id'), arg);
        });
        //让后退页面回到之前的高度
        function getMemory(id, arg) {
            if($.inArray(id, $.custom.correctPages) > -1) {
                return;
            }
...
+function ($) {
    ...
    $.initPage = function (page) {
        ...
        //var super_modal = $('.super-modal-overlay');
        //阻止超级modal滚动
        //super_modal.off('touchmove').on("touchmove", function(e) {
        //    e.preventDefault();
        //}, false);

        //阻止超级modal子元素冒泡
        //super_modal.children().off('click').on('click', function(e) {
        //    stopPropagation(e);
        //});
    };
